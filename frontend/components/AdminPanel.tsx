...
import { BattleCampaignsConsole } from './BattleCampaignsConsole';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const [adminSubTab, setAdminSubTab] = useState<'finance_users' | 'battle_campaigns'>('battle_campaigns');
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-7 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900">後台對帳、免費會員與四大戰役推播</h1>
      ...
      {/* 後台內部次級導覽：切換「四大戰役行推控制台」與「財務/會員後台」 */}
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2">
        <button
          onClick={() => setAdminSubTab('battle_campaigns')}
          className={`flex items-center space-x-1.5 px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition ${
            adminSubTab === 'battle_campaigns'
              ? 'bg-slate-900 text-amber-300 shadow-md'
              : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
          }`}
        >
          <Flame className="w-4 h-4 text-amber-400" />
          <span>四大戰役擴散法推播控制</span>
        </button>

        <button
          onClick={() => setAdminSubTab('finance_users')}
          className={`flex items-center space-x-1.5 px-4 py-2 rounded-2xl text-xs sm:text-sm font-bold transition ${
            adminSubTab === 'finance_users'
              ? 'bg-slate-900 text-white shadow-md'
              : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100'
          }`}
        >
          <Settings className="w-4 h-4 text-cyan-500" />
          <span>電匯對帳、免費會員與 Agent 設定</span>
        </button>
      </div>

      {/* 依分頁呈現對應面板 */}
      {adminSubTab === 'battle_campaigns' && (
        <BattleCampaignsConsole />
      )}
